...
check_rstudio_version <- function(check_for_updates = TRUE) {
    if (requireNamespace("rstudioapi", quietly = TRUE) && interactive()) {
        ...
        message("RStudio ", current_version)
        if (check_for_updates) {
            if (!oddments:::has_internet()) {
...
#' @rdname check_versions
#' @name check_pandoc_version
#' @export
check_pandoc_version <- function(check_for_updates = TRUE) {
    if(!interactive()) return(invisible())
    if(!requireNamespace("rmarkdown", quietly = TRUE)){
        message("can't check pandoc version - rmarkdown package not installed")
        return(invisible())
    }
    if(!rmarkdown::pandoc_available()){
        current_version <- numeric_version(0)
        message("pandoc not installed")
    }else{
        current_version <- rmarkdown::pandoc_version()
        message("pandoc ", current_version)
    }
    if (!check_for_updates) return(invisible())
    if (!oddments:::has_internet()) {
        message("can't check latest pandoc version - no internet connection")
        return(invisible())
    }
    url <- "https://pandoc.org/releases.html"
    url_dump <- try(suppressWarnings({
        url %>% readLines(n = 1000, warn = FALSE)
    }), TRUE)
    if (inherits(url_dump, "try-error")) {
