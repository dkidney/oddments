...
#' skim(esoph)
#' skim(attenu)
#' skim(nycflights13::airlines)
#' skim(nycflights13::airports)
#' skim(nycflights13::flights)
#' skim(nycflights13::planes)
#' skim(nycflights13::weather)
#' }
...
skim = function(x,
    ...
    out$type = x %>% map_chr(tibble::type_sum)
    if(verbose){
        temp = out$type %>% table
    ...
    out$p_na = out$na / nrow(x)
    if(verbose){
        any_na = (out$na > 0) %in% TRUE
        if(any(any_na)){
    ...
    if(verbose){
        sum_nan = sum(out$nan > 0, na.rm = TRUE)
        if(sum_nan > 0){
            warn(paste(sum_nan, "dbl/int vars have NaN values"))
        }
    }
    ...
    if(verbose){
        sum_inf = sum(out$inf > 0, na.rm = TRUE)
        if(sum_inf > 0){
            warn(paste(sum_inf, "dbl/int vars have infinite values"))
        }
    }
    ...
    if(verbose){
        # strictly positive
        temp = out %>%
            filter(.data$pos > 0 & !.data$zero > 0 & !.data$neg > 0)
        if(nrow(temp) > 0){
